...
class LiixAiPanelProvider {
    ...
    async handleReadActiveFile(modelId) {
        ...
        this.postAssistantMessage(`Fichier actif lu.\n\n` +
    ...
    async handleAnalyzeErrors(modelId) {
        const activeUri = vscode.window.activeTextEditor?.document.uri;
        const buildLogSummary = await this.readBuildLogSummary();
        const diagnosticSummary = (0, aiErrorParser_1.summarizeDiagnostics)(activeUri);
        ...
        this.postAssistantMessage(`Analyse erreurs (${summary.source}).\n\n` +
            `Erreurs: ${summary.errors}\n` +
            `Warnings: ${summary.warnings}\n\n` +
            `${details}\n\n` +
            response.content);
    }
    async readBuildLogSummary() {
        ...
        return (0, aiErrorParser_1.parseBuildLog)(log);
    }
    ...
    getHtml() {
        const modelDisplayLabels = aiModels_1.liixAiModels.reduce((labels, model) => {
            labels[model.id] = `${model.label} - Training`;
            return labels;
        }, {});
        const modelDescriptions = aiModels_1.liixAiModels.reduce((descriptions, model) => {
            descriptions[model.id] = model.description;
            return descriptions;
        }, {});
        const modelCompactLabels = {
            "liix-code-0-1": "Liix 0.1",
            "liix-code-0-1-mini": "Mini",
            "liix-code-a1": "A1"
        };
        const modelOptions = aiModels_1.liixAiModels.map((model) => (`<option value="${escapeHtml(model.id)}" title="${escapeHtml(`${model.label} - Training - ${model.description}`)}">${escapeHtml(modelCompactLabels[model.id] || `${model.label} - Training`)}</option>`)).join("");
        return `<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8" />

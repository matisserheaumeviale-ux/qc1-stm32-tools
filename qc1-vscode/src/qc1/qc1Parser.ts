export interface QC1DiagnosticEntry {
  file?: string;
  line?: number;
  column?: number;
...
export interface QC1ParseResult {
  errors: number;
  warnings: number;
  hasBuildFailed: boolean;
  hasFlashFailed: boolean;
  elfGenerated: boolean;
  binGenerated: boolean;
  flashUsage: string;
  ramUsage: string;
  diagnostics: QC1DiagnosticEntry[];
  explanation: string;
}
...
export function parseQc1Output(output: string): QC1ParseResult {
  const lines = output.split(/\r?\n/);
  const diagnostics: QC1DiagnosticEntry[] = [];
